...
public class ConvexHull {
    ...
    public IntList findHullPar(int f, int t, int k, IntList coHull, int n, IntList test) {
        ...
        System.out.println(tempCoHull.size());
        for (int i = 0; i < tempCoHull.size(); i++) {
            System.out.println(tempCoHull.get(i));
        }

        for (int i = 0; i < test.size(); i++) {
            int thing = test.get(i);
            boolean notInList = true;
            for (int j = 0; j < tempCoHull.size(); j++) {
                if (thing == tempCoHull.get(j)) {
                    notInList = false;
                    break;
                }
            }
            if (notInList)
                System.out.println("NOT IN LIST " + "x-" + x[thing] + " y-" + y[thing]);
        }
    ...
    public static void main(String[] args) {
         ...
         * 
         * String header = String.format("%-10s %-10s %-10s %-10s", "n", "seq(ms)",
         * "par(ms)", "Spdup");
         * System.out.println(header);
         * System.out.println(String.format("%-10d %-10.3f %-10.3f %-10.3f", n, timeSeq,
         * timePar, speedUp));
         * }
        ...
        NPunkter17 nPunkter17 = new NPunkter17(10000, seed);
        ConvexHull ch = new ConvexHull(10000, seed, nPunkter17);
        IntList coHullSeq = new IntList();
        IntList coHullPar = new IntList();

        ch.findHullSeq(ch.MAX_X, ch.MIN_X, coHullSeq, 0, 10000);
        IntList asd = ch.findHullPar(ch.MAX_X, ch.MIN_X, k, coHullPar, 10000, coHullSeq);

        Oblig4Precode precode = new Oblig4Precode(ch, coHullSeq);
        precode.writeHullPoints();
        precode.drawGraph();

        precode = new Oblig4Precode(ch, asd);
        precode.writeHullPointsPar();
        precode.drawGraph();
    ...
    public static void checkCoHulls(IntList coHull1, IntList coHull2) {
        assert coHull1.size() == coHull2.size() : String.format("coHulls has different sizes\nCoHull1: %s\nCoHull2: %s",
                coHull1.size(), coHull2.size());
        int n = coHull1.size();
        for (int i = 0; i < n; i++) {
