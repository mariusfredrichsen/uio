...
for i in range(antall_simuleringer):
    ...
    sum_for += kampliste[0].mål_hjemme()
    sum_mot += kampliste[0].mål_borte()

    if kampliste[0].mål_hjemme() > kampliste[0].mål_borte():
        hjemmeseier += 1

snitt_for = round(0.5*sum_for/antall_simuleringer, 2)
snitt_mot = round(0.5*sum_mot/antall_simuleringer, 2)

print(snitt.navn(), "   ", snitt_for, "   ", snitt_mot)

sum_mål += sum_for + sum_mot
sum_kamper = antall_simuleringer
hjemmeseier_prosent = round(100 * hjemmeseier / sum_kamper, 2)
print()
