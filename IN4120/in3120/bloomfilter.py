...
class BloomFilter:
    ...
    for educational demonstration purposes. A Bloom filter is a space-efficient
    ...
    # Precompute a lookup table that for each possible byte value counts how many bits are set.
    # So-called population counts ("popcounts") are sometimes also referred to as Hamming weights.
    ...
    def estimated_count(self) -> int:
        """
        Returns an estimate of the number of items that have been added to the Bloom filter.

        Similar expressions exist for approximating the size of the intersection and union of
        two sets, when the sets are represented as Bloom filters. For details, see
        https://en.wikipedia.org/wiki/Bloom_filter#Approximating_the_number_of_items_in_a_Bloom_filter.
        """
        ones = sum(BloomFilter._POPCOUNT[byte] for byte in self._bits)
        return max(0, round(-(self._m / self._k) * math.log(1 - (ones / self._m))))
