...
class TestWordShingleGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._tokenizer = in3120.WordShingleGenerator(2, tokenizer, normalizer)

    def test_strings(self):
        self.assertListEqual(list(self._tokenizer.strings("")), [])
        self.assertListEqual(list(self._tokenizer.strings("Foo")), ["foo"])
        self.assertListEqual(list(self._tokenizer.strings("Foo  the")), ["foo the"])
        self.assertListEqual(list(self._tokenizer.strings("Foo the  BAR")), ["foo the", "the bar"])

    def test_tokens(self):
        self.assertListEqual(list(self._tokenizer.tokens("Foo")), [("foo", (0, 3))])
        self.assertListEqual(list(self._tokenizer.tokens("Foo  the")), [("foo the", (0, 8))])
        self.assertListEqual(list(self._tokenizer.tokens("Foo the  BAR")), [("foo the", (0, 7)), ("the bar", (4, 12))])

    def test_spans(self):
        self.assertListEqual(list(self._tokenizer.spans("foo")), [(0, 3)])
        self.assertListEqual(list(self._tokenizer.spans("foo the")), [(0, 7)])
        self.assertListEqual(list(self._tokenizer.spans("foo the bar")), [(0, 7), (4, 11)])
