...
class TestDocumentPipeline(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        us_state_capitals = in3120.SimpleTrie.from_strings(["Phoenix", "New York City", "Sacramento", "Little Rock"], analyzer)
        self._capitalfinder = in3120.StringFinder(us_state_capitals, analyzer)
    ...
    def _compute_gog_from_foo(self, document: in3120.Document) -> in3120.Document:
        document["gog"] = ", ".join(result.match for result in self._extractor.extract(document["foo"]))
        return document

    def _compute_bib_from_foo(self, document: in3120.Document) -> in3120.Document:
        document["bib"] = ", ".join(result.match for result in self._capitalfinder.scan(document["foo"]))
        return document
