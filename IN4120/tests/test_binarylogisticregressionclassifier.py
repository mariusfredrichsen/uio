...
class TestBinaryLogisticRegressionClassifier(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._fields = ["body"]
        inverted_index = in3120.DummyInMemoryInvertedIndex(corpus, self._fields, analyzer)
        self._training_set = corpus.split("is_china", lambda v: ["china"] if "Yes" in v else ["not china"])
        self._vectorizer = in3120.Vectorizer(corpus, inverted_index, in3120.SimpleTrie())
    ...
    def test_stopping_early_works_and_loss_decreases(self):
        ...
        options = in3120.BinaryLogisticRegressionClassifier.Options(epochs=314)
        classifier.train(self._training_set, self._fields, monitor, options)
    ...
    def _verify_result(self, classifier: in3120.BinaryLogisticRegressionClassifier, buffer: str, china: bool):
        ...
        self.assertListEqual(expected, [result.category for result in results])
    ...
    def test_basic_training_and_classification_works(self):
        classifier = in3120.BinaryLogisticRegressionClassifier(self._vectorizer)
        options = in3120.BinaryLogisticRegressionClassifier.Options(epochs=100)
        classifier.train(self._training_set, self._fields, None, options)
    ...
    def test_training_barfs_on_tasks_with_more_than_two_classes(self):
        ...
        for training_set in (barf1, barf3):
            ...
            with self.assertRaises(AssertionError):
                classifier.train(training_set, self._fields, None, options)
