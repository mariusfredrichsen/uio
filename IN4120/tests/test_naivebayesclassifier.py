...
class TestNaiveBayesClassifier(unittest.TestCase):
    ...
    def setUp(self):
        self._analyzer1 = in3120.SimpleAnalyzer()
        self._analyzer2 = in3120.Analyzer(in3120.SimpleNormalizer(), in3120.ShingleGenerator(3))
    ...
    def test_china_example_from_textbook(self):
        ...
        classifier = in3120.NaiveBayesClassifier(training_set, ["body"], self._analyzer1)
    ...
    def test_language_detection_trained_on_some_news_corpora(self):
        training_set = {language: in3120.CorpusLoader.from_files(in3120.InMemoryCorpus(), [f"../data/{language}.txt"]) for language in ["en", "no", "da", "de"]}
        classifier = in3120.NaiveBayesClassifier(training_set, ["body"], self._analyzer1)
        self._classify_buffer_and_verify_top_categories("Vil det riktige språket identifiseres? Dette er bokmål.", classifier, ["no"])
        self._classify_buffer_and_verify_top_categories("I don't believe that the number of tokens exceeds a billion.", classifier, ["en"])
        self._classify_buffer_and_verify_top_categories("De danske drenge drikker snaps!", classifier, ["da"])
        self._classify_buffer_and_verify_top_categories("Der Kriminalpolizei! Haben sie angst?", classifier, ["de"])
    ...
    def test_predict_movie_genre_from_movie_title(self):
        ...
        classifier = in3120.NaiveBayesClassifier(training_set, ["title"], self._analyzer2)
        self._classify_buffer_and_verify_top_categories("friday the 13th 1408 crawlspace", classifier, ["Horror"])
        self._classify_buffer_and_verify_top_categories("kung fu sausage panda university", classifier, ["Animation"])

    def test_predict_name_of_search_engine_from_description(self):
        engines = in3120.CorpusLoader.from_files(in3120.InMemoryCorpus(), ["../data/docs.json"])
        training_set = engines.split("title")
        classifier = in3120.NaiveBayesClassifier(training_set, ["body"], self._analyzer2)
        self._classify_buffer_and_verify_top_categories("duckie duckie privacy", classifier, ["Duck Duck Go"])
        self._classify_buffer_and_verify_top_categories("jeeeeeves webb", classifier, ["Ask"])
    ...
    def test_uses_yield(self):
        ...
        classifier = in3120.NaiveBayesClassifier(training_set, ["a"], self._analyzer1)
