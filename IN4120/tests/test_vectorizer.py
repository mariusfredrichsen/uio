...
class TestVectorizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._corpus = in3120.CorpusLoader.from_files(in3120.InMemoryCorpus(), ["../data/en.txt"])
        self._fields = ["title", "body"]
        self._inverted_index = in3120.DummyInMemoryInvertedIndex(self._corpus, self._fields, self._analyzer)
        self._document = self._corpus[171]  # "A date for that hearing has not yet been set."
        self._stopwords = in3120.SimpleTrie.from_strings(["the", "of", "in"], self._analyzer)
