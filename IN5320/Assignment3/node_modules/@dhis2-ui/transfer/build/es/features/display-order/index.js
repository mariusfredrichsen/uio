...
When('the user deselects one selected option', () => {
  ...
  cy.get('{transfer-pickedoptions}').find('.highlighted').then($el => $el.toArray().map(extractOptionFromElement)).as('deselectedOptions');
...
When('the user deselects multiple selected options', () => {
  ...
  .then($options => {
    ...
    return [$arr[2], $arr[0]];
  }).each($option => cy.wrap($option).clickWith('cmd')).then($options => cy.wrap($options.toArray().map(extractOptionFromElement)).as('deselectedOptions'));
  cy.get('{transfer-actions-removeindividual}').click();
});
When('the user selects one option', () => {
  cy.get('{transfer-sourceoptions} {transferoption}').first().click();
  cy.get('{transfer-sourceoptions}').find('.highlighted').then($el => $el.toArray().map(extractOptionFromElement)).as('selectedOptions');
...
When('the user selects multiple options', () => {
  ...
  .then($options => {
    const $arr = $options.toArray();
    return [$arr[4], $arr[0]];
  }).each($option => cy.wrap($option).clickWith('ctrl')).then($options => {
    cy.wrap($options.toArray().map(extractOptionFromElement)).as('selectedOptions');
  });
  cy.get('{transfer-actions-addindividual}').click();
});
