...
const calendar = new _nepaliCalendar.NepaliCalendar();
describe('from nepali to gregorian', () => {
  it('should convert from nepali calendar to gregorian', () => {
    ...
    const result = _polyfill.Temporal.Calendar.from(calendar).dateFromFields(nepaliDate);
  ...
  it('should convert from nepali calendar to gregorian for the last n years', () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const date = _polyfill.Temporal.Calendar.from(calendar).dateFromFields(nepaliDate);
  ...
  it('should throw an error if nepali year is out of range of supported dates', () => {
    expect(() => _polyfill.Temporal.Calendar.from(calendar).dateFromFields({
      year: 1969,
      month: 7,
      day: 1
  ...
  it('should convert a year/month from nepali calendar to gregorian', () => {
    ...
    const result = _polyfill.Temporal.PlainYearMonth.from(nepaliDate);
...
describe('from greogorian to nepali', () => {
  it('should convert from gregorian calendar to nepali', () => {
    const nepaliDate = _polyfill.Temporal.PlainDate.from('2022-10-18').withCalendar(calendar);
  ...
  it('should convert from gregorian calendar to nepali for the last/next n years', () => {
    ...
    const nepaliStartYear = 2100;
    for (let i = 0; i < 100; i++) {
      const nepaliDate = _polyfill.Temporal.PlainDate.from(`${gregorianStartYear - i}-10-18`).withCalendar(calendar);
      expect(nepaliDate.year).toEqual(nepaliStartYear - i);
    }
  });
  it('should throw an error if nepali year is out of range of supported dates', () => {
    expect(() => {
      _polyfill.Temporal.PlainDate.from('2044-10-18').withCalendar(calendar).year;
    }).toThrow('Conversions are only possible between 1970 and 2100 in Nepali calendar');
  });
});
...
describe('some random conversions nepali <-> gregorian', () => {
  ...
  it.each(RANDOM_CONVERSION_MAPS)(`it should convert from nepali to gregorian and vice versa`, _ref => {
    ...
    const isoResult = _polyfill.Temporal.Calendar.from(calendar).dateFromFields(nepaliDate);
    const {
      isoYear,
      isoMonth,
      isoDay
    ...
    const nepaliResult = _polyfill.Temporal.PlainDate.from(gregorianDate).withCalendar(calendar);
