...
When("the user clicks the 'move all to selected list' button", () => {
  cy.get('{transfer-sourceoptions} {transferoption}').then($options => $options.toArray().map(extractOptionFromElement)).as('itemsToBeSelected');
  cy.get('{transfer-actions-addall}').click();
});
When("the user clicks the 'move all to options list' button", () => {
  cy.get('{transfer-pickedoptions} {transferoption}').then($options => $options.toArray().map(extractOptionFromElement)).as('itemsToBeDeselected');
  cy.get('{transfer-actions-removeall}').click();
});
When('the user double clicks an item in the options list', () => {
  cy.get('{transfer-sourceoptions} {transferoption}').first().dblclick().then(extractOptionFromElement).as('doubleClickedPlainOption');
});
