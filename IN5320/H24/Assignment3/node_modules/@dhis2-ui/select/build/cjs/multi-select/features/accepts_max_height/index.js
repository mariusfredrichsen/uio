"use strict";

var _cypressCucumberPreprocessor = require("@badeball/cypress-cucumber-preprocessor");
(0, _cypressCucumberPreprocessor.Given)('a MultiSelect with more than ten options is rendered', () => {
  cy.visitStory('MultiSelect', 'With more than ten options');
});
(0, _cypressCucumberPreprocessor.Given)('a MultiSelect with more than three options and a 100px max-height is rendered', () => {
  cy.visitStory('MultiSelect', 'With more than three options and a 100 px max-height');
});
(0, _cypressCucumberPreprocessor.Then)('has the default max-height', () => {
  cy.get('[data-test="dhis2-uicore-select-menu-menuwrapper"]').should('have.css', 'max-height', '280px');
});
(0, _cypressCucumberPreprocessor.Then)('has a max-height of 100px', () => {
  cy.get('[data-test="dhis2-uicore-select-menu-menuwrapper"]').should('have.css', 'max-height', '100px');
});