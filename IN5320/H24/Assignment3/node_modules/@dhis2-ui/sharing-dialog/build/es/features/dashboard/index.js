...
Given('the two dashboard tabs are visible', () => {
  ...
  cy.contains('button', 'Apply sharing to dashboard visualizations').should('be.visible');
});
When('the apply sharing tab is clicked', () => {
  cy.intercept('GET', '/api/38/dashboards/id?fields=dashboardItems%5Btype%5D', {
    body: dashboards
  });
...
When('the apply sharing button is clicked', () => {
  ...
  cy.contains('[data-test="dhis2-uicore-button"]', 'Apply sharing to dashboard visualizations').click();
});
...
Then('the correct counts should be displayed', () => {
  ...
  cy.contains('Number of visualizations on this dashboard that will potentially get updated sharing settings: 4. The number of users or groups that these updated settings will apply to: 2.').should('be.visible');
});
Then('the apply sharing button should be visible', () => {
  cy.contains('[data-test="dhis2-uicore-button"]', 'Apply sharing to dashboard visualizations').should('be.visible');
});
Then('a result message should be displayed', () => {
  cy.contains('Successfully updated sharing for all visualizations.').should('be.visible');
});
Then('an alert with the error message should be displayed', () => {
  cy.contains('An unknown error occurred - Internal Server Error (500)').should('be.visible');
});