...
(0, _cypressCucumberPreprocessor.Given)('there is enough space between the reference element top and the body top to fit the Tooltip', () => {
  cy.visitStory('Tooltip', 'Default');
});
(0, _cypressCucumberPreprocessor.Given)('there is not enough space above or below the anchor to fit the Tooltip', () => {
  cy.visitStory('Tooltip', 'Shifting Position');
});
(0, _cypressCucumberPreprocessor.When)('the mouse cursor enters the anchor', () => {
  cy.get('[data-test="dhis2-uicore-tooltip-reference"]').trigger('mouseover', 'top');
});
(0, _cypressCucumberPreprocessor.Then)('the Tooltip is rendered above the anchor', () => {
...
(0, _cypressCucumberPreprocessor.Then)('the Tooltip stays visible', () => {
  cy.clock();
  cy.tick(800);
  cy.get('[data-test="dhis2-uicore-tooltip-content"]').should('exist');
});
(0, _cypressCucumberPreprocessor.Then)('the Tooltip is rendered on top of the anchor', () => {
  cy.get('[data-test="dhis2-uicore-tooltip-reference"]').then($ref => {
    const refPos = $ref[0].getBoundingClientRect();
    cy.get('[data-test="dhis2-uicore-tooltip-content"]').then($content => {
