...
var _nepaliCalendarData = require("./nepaliCalendarData");
/**
 ...
 */
class NepaliCalendar extends _polyfill.Temporal.Calendar {
  constructor() {
    super('iso8601');
  }
  toString() {
  ...
  daysInMonth(date) {
    ...
    return _nepaliCalendarData.NEPALI_CALENDAR_DATA[year][month];
  }
  month(date) {
    var _isoToNepali3;
    const {
  ...
  dateFromFields(fields) {
    ...
    return new _polyfill.Temporal.PlainDate(year, month, day, this);
  }
  yearMonthFromFields(fields) {
    ...
    return new _polyfill.Temporal.PlainYearMonth(year, month, this, day);
  }
  monthDayFromFields(fields) {
    ...
    return new _polyfill.Temporal.PlainMonthDay(day, month, this, year);
  }
}
exports.NepaliCalendar = NepaliCalendar;
const supportedNepaliYears = Object.keys(_nepaliCalendarData.NEPALI_CALENDAR_DATA);
...
const _nepaliToIso = fields => {
  ...
  const gregorianYear = nepaliYear - (nepaliMonth > 9 || monthCounter === 9 && nepaliDay >= _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][0] ? 56 : 57);
  ...
  while (monthCounter !== 9) {
    ...
    gregorianDayOfYear += _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][monthCounter];
    monthCounter--;
  }
  ...
  if (nepaliMonth === 9) {
    ...
    gregorianDayOfYear += nepaliDay - _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][0];
    ...
    if (gregorianDayOfYear < 0) {
      gregorianDayOfYear += _polyfill.Temporal.PlainDate.from({
        year: gregorianYear,
        day: 1,
        month: 1
      }).daysInYear;
    ...
    gregorianDayOfYear += _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][9] - _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][0];
  }
  const result = _polyfill.Temporal.PlainDate.from({
    year: gregorianYear,
    month: 1,
    day: 1
  }).add({
...
const _isoToNepali = isoDate => {
  ...
  const gregorianDate = _polyfill.Temporal.PlainDate.from({
    year: isoDate.year,
    month: isoDate.month,
    day: isoDate.day
  });
  ...
  if (!_nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear]) {
    throw new Error(`Conversions are only possible between ${firstSupportedNepaliYear} and ${lastSupportedNepaliYear} in Nepali calendar`);
  }
  ...
  const dayOfFirstJanInPaush = _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][0];
  ...
  let daysSinceJanFirstToEndOfNepaliMonth = _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][nepaliMonth] - dayOfFirstJanInPaush + 1;
  ...
  while (gregorianDayOfYear > daysSinceJanFirstToEndOfNepaliMonth) {
    ...
    daysSinceJanFirstToEndOfNepaliMonth += (_NEPALI_CALENDAR_DATA = _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear]) === null || _NEPALI_CALENDAR_DATA === void 0 ? void 0 : _NEPALI_CALENDAR_DATA[nepaliMonth];
  }
  if (!_nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear]) {
    throw new Error(`Conversions are only possible between ${firstSupportedNepaliYear} and ${lastSupportedNepaliYear} in Nepali calendar`);
  }
  ...
  const nepaliDayOfMonth = _nepaliCalendarData.NEPALI_CALENDAR_DATA[nepaliYear][nepaliMonth] - (daysSinceJanFirstToEndOfNepaliMonth - gregorianDayOfYear);
  return {
    year: nepaliYear,
    month: nepaliMonth,
